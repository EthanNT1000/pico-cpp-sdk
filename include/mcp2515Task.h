...
class Mcp2515Task : public Mcp2515, public TaskInterface {
 public:
    ...
    void rxBuffer0ReadInTask(uint32_t* canId, uint8_t* buf, uint8_t* len);
    void rxBuffer1ReadInTask(uint32_t* canId, uint8_t* buf, uint8_t* len);
 ...
 private:
    ...
    SemaphoreHandle_t _rx0Semaphore { nullptr };  // given by canService on RX0 complete
    SemaphoreHandle_t _rx1Semaphore  { nullptr };  // given by canService on RX1 complete
